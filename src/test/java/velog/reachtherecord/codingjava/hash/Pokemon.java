...
public class Pokemon {
    ...
    class Solution {
        public int solution(int[] nums) {
            int answer = 0;
            Map<Integer, Integer> map = new HashMap<>();    //포켓몬 담을 맵 선언
            for (int i = 0; i < nums.length; i++) {
                if (map.containsKey(nums[i])) { //맵에 해당 포켓몬이 담겨있으면 기존값 +1, 아닐경우 맵에 새로 추가
                    map.put(nums[i], map.get(nums[i]) + 1);
                } else {
                    map.put(nums[i], 1);
                }
            }
            answer = map.size() < nums.length / 2 ? map.size() : nums.length / 2;   //전체 포켓몬 수를 2로 나눈 값이 전체 종류보다 크면 종류별로 하나씩 (맵 사이즈), 아닐 경우 포켓몬을 반으로 나눈 값을 리턴
